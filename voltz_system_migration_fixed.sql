-- Voltz System Migration (Fixed - No Trigger Loops)
-- This migration fixes the dual-tracking Voltz system without causing infinite loops

BEGIN;

-- STEP 0: Temporarily disable profile completion triggers to prevent loops
ALTER TABLE public.profiles DISABLE TRIGGER trigger_update_profile_completion_profiles;
...
-- Only update where total_voltz_earned is 0 to avoid re-running
UPDATE public.profiles 
SET total_voltz_earned = COALESCE(xp, 0)
WHERE total_voltz_earned = 0 AND xp IS NOT NULL;

-- Step 3: Ensure spendable_voltz has proper default
ALTER TABLE public.profiles 
ALTER COLUMN spendable_voltz SET DEFAULT 100;

UPDATE public.profiles 
SET spendable_voltz = COALESCE(spendable_voltz, 100)
WHERE spendable_voltz IS NULL;

-- Step 4: Add transaction_type column to xp_ledger if it doesn't exist
DO $$
BEGIN
...
-- Step 5: Create/replace the unified Voltz management function
...
BEGIN
  ...
  IF p_transaction_type = 'earned' THEN
    -- Temporarily disable triggers to prevent loops during updates
  ...
  ELSIF p_transaction_type = 'spent' THEN
    ...
    BEGIN
      ...
      -- Temporarily disable triggers to prevent loops
      SET session_replication_role = replica;
...
BEGIN
  ...
  DECLARE
    ...
    voltz_needed_for_next_level INTEGER := 20 - progress_in_level;
  BEGIN
    RETURN QUERY SELECT
...
-- Step 7: Update existing Voltz-related functions to use new system
...
-- Step 8: Update quiz trigger to use new Voltz system
...
-- Step 10: Re-enable the profile completion trigger
ALTER TABLE public.profiles ENABLE TRIGGER trigger_update_profile_completion_profiles;
