...
import { View, Text, TouchableOpacity, StyleSheet, Alert, ScrollView, Modal, Linking, Switch, ActivityIndicator } from 'react-native';
...
const SettingsModal: React.FC<SettingsModalProps> = ({ visible, onClose, navigateTo, signOut, deleteAccount }) => {
  ...
  // Check notification permission and privacy status on mount
  useEffect(() => {
    if (visible && user) {
  ...
  const checkPrivacyStatus = async () => {
    ...
    try {
      ...
      console.error('Error checking privacy status:', e);
    } finally {
  ...
  const handlePrivacyToggle = async (value: boolean) => {
    ...
    try {
      ...
      // Update local storage or global state if needed
      // Alert.alert('Success', `Your account is now ${value ? 'Private' : 'Public'}`);
    } catch (e) {
          ...
          <View style={dynamicStyles.section}>
            ...
            <View style={dynamicStyles.settingCard}>
              <View style={[dynamicStyles.settingItem, dynamicStyles.settingItemLast]}>
                ...
                <View style={dynamicStyles.settingContent}>
                  <Text style={dynamicStyles.settingTitle}>Private Account</Text>
                  <Text style={dynamicStyles.settingDescription}>
                    {isPrivate
                      ? 'Only followers can see your profile and posts. You must approve new followers.'
