-- Migration 009: Create Hybrid Search Functions for Autocomplete
-- This migration creates the search functions that power LinkedIn-style autocomplete
-- Uses prefix matching, fuzzy search, and full-text search with relevance ranking

BEGIN;

-- Function for fast company search with hybrid ranking
...
BEGIN
  ...
  WITH search_results AS (
    ...
    SELECT DISTINCT
      ...
      1.0 + (c.popularity_score::DECIMAL / 100) as score,
    ...
    SELECT DISTINCT
      ...
      c.country_code,
      c.employee_count_range,
      0.9 + (c.popularity_score::DECIMAL / 100) as score,
    ...
    SELECT DISTINCT
      ...
      c.country_code,
      c.employee_count_range,
      0.6 + similarity(c.name_normalized, normalized_query) + (c.popularity_score::DECIMAL / 200) as score,
      'fuzzy' as match_type
    FROM companies c
    WHERE c.status = 'active'
    ...
    SELECT DISTINCT
      ...
      c.country_code,
      c.employee_count_range,
      0.4 + ts_rank(c.search_vector, tsquery_obj) + (c.popularity_score::DECIMAL / 200) as score,
...
-- Function for fast occupation search with hybrid ranking
...
BEGIN
  ...
  WITH search_results AS (
    ...
    SELECT DISTINCT
      ...
      o.category,
      o.salary_range_gbp,
      1.0 + (o.popularity_score::DECIMAL / 100) as score,
      'exact_prefix' as match_type
    FROM occupations o
    ...
    SELECT DISTINCT
      ...
      o.category,
      o.salary_range_gbp,
      0.9 + (o.popularity_score::DECIMAL / 100) as score,
    ...
    SELECT DISTINCT
      ...
      o.category,
      o.salary_range_gbp,
      0.6 + similarity(o.title_normalized, normalized_query) + (o.popularity_score::DECIMAL / 200) as score,
      'fuzzy' as match_type
    FROM occupations o
    ...
    SELECT DISTINCT
      ...
      o.category,
      o.salary_range_gbp,
      0.4 + ts_rank(o.search_vector, tsquery_obj) + (o.popularity_score::DECIMAL / 200) as score,
      'fulltext' as match_type
    FROM occupations o
...
-- Function for mixed/universal search (companies + occupations)
...
-- Function to record search selections for popularity tracking
CREATE OR REPLACE FUNCTION record_selection(
  entity_type TEXT, -- 'company' or 'occupation'
  entity_id UUID,
...
BEGIN
  IF entity_type = 'company' THEN
    ...
    SET popularity_score = popularity_score + 1,
        updated_at = NOW()
    WHERE id = entity_id;
  ELSIF entity_type = 'occupation' THEN
    UPDATE occupations 
    SET popularity_score = popularity_score + 1,
        updated_at = NOW()
    WHERE id = entity_id;
...
-- Function to get popular companies for trending/suggestions
...
-- Function to get popular occupations for trending/suggestions
CREATE OR REPLACE FUNCTION get_popular_occupations(
  result_limit INTEGER DEFAULT 10,
  category_filter TEXT DEFAULT NULL
)
RETURNS TABLE (
...
BEGIN
  ...
  SELECT 
    ...
    o.popularity_score
  FROM occupations o
  WHERE (category_filter IS NULL OR o.category = category_filter)
  ORDER BY o.popularity_score DESC, o.title
  LIMIT result_limit;
END;
$$ LANGUAGE plpgsql;

-- Grant execute permissions to authenticated users
GRANT EXECUTE ON FUNCTION search_companies(TEXT, INTEGER, TEXT) TO authenticated;
GRANT EXECUTE ON FUNCTION search_occupations(TEXT, INTEGER, TEXT) TO authenticated;
GRANT EXECUTE ON FUNCTION search_mixed(TEXT, INTEGER) TO authenticated;
GRANT EXECUTE ON FUNCTION record_selection(TEXT, UUID, TEXT) TO authenticated;
GRANT EXECUTE ON FUNCTION get_popular_companies(INTEGER, TEXT) TO authenticated;
GRANT EXECUTE ON FUNCTION get_popular_occupations(INTEGER, TEXT) TO authenticated;
...
-- Search companies
SELECT * FROM search_companies('micro', 5);
SELECT * FROM search_companies('google', 5, 'US');

-- Search occupations  
SELECT * FROM search_occupations('software', 5);
SELECT * FROM search_occupations('manager', 5, 'Product');

-- Mixed search
SELECT * FROM search_mixed('product manager', 8);

-- Record user selection (for popularity tracking)
SELECT record_selection('company', 'company-uuid-here');
SELECT record_selection('occupation', 'occupation-uuid-here', 'software engineer');

-- Get popular suggestions
SELECT * FROM get_popular_companies(10, 'GB');
SELECT * FROM get_popular_occupations(10, 'Engineering');
